...
const file = async (): Promise<void> => {
  ...
  if (!target || !targetPath || !targetFolderPath)
    return alert.error('You need to open a file to diff first');
  ...
  const isCompatible = (filePath: string) =>
    isBinaryPath(filePath) === isBinary;
  const isTarget = (filePath: string) => filePath === targetPath;

  const filesUntitled =
    isTextual && options.showUntitledFiles && options.showOpenFiles
      ? getOpenUntitledFiles()
      : [];
  const filesUntitledCompatible = filesUntitled.filter(
    (file) => !isTarget(file.path),
  );
  const filesUntitledSorted = sortByPath(
    filesUntitledCompatible,
    (file) => file.path,
  );

  const filesOpen = options.showOpenFiles ? getOpenFilesPaths() : [];
  const filesOpenCompatible = filesOpen.filter(
    (filePath) => isCompatible(filePath) && !isTarget(filePath),
  );
  const filesOpenSorted = sortByPath(
    filesOpenCompatible,
    (filePath) => filePath,
  );

  const filesFound =
    rootPath && options.showFoundFiles
      ? await getFiles(
          rootPath,
          options.include,
          options.exclude,
      ...
      : [];
  const filesFoundCompatible = filesFound.filter(
    (filePath) => isCompatible(filePath) && !isTarget(filePath),
  );
  const filesFoundSorted = sortByPath(
    filesFoundCompatible,
    (filePath) => filePath,
  );

  const filesFoundRelativeSorted =
    !isUntitled && options.showFoundRelativeFiles ? filesFoundSorted : [];
  ...
  const leftPath = !isString(target)
    ? await getFileTemp(target.path, target.content)
    : targetPath;
  const rightPath =
    'file' in item
      ? await getFileTemp(item.file.path, item.file.content)
      : item.filePath;
  ...
  const title =
    leftName !== rightName
      ? `${leftName} ↔ ${rightName}`
      : `${leftName} (${leftPath}) ↔ ${rightName} (${rightPath})`; //TODO: Improve this, using the whole path is pretty snoisy
