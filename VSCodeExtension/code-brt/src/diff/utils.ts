...
const getFilesExclude = (): string[] => {
  ...
  const globs = Object.entries(excludes)
...
const getOptions = (): Options => {
  ...
  const exclude =
    isArray(config?.exclude) && config.exclude.every(isString)
      ? config.exclude
      : getFilesExclude();
  const ignore =
    isArray(config?.ignore) && config.ignore.every(isString)
      ? config.ignore
      : ['.gitignore'];
  const include =
    isArray(config?.include) &&
    config.include.every(isString) &&
    config.include.length
      ? config.include
      : ['**/*'];
  const showUntitledFiles = isBoolean(config?.showUntitledFiles)
    ? config.showUntitledFiles
    : true;
  const showOpenFiles = isBoolean(config?.showOpenFiles)
    ? config.showOpenFiles
    : true;
  const showFoundFiles = isBoolean(config?.showFoundFiles)
    ? config.showFoundFiles
    : true;
  const showFoundRelativeFiles = isBoolean(config?.showFoundRelativeFiles)
    ? config.showFoundRelativeFiles
    : false;
